...
df_lw = get_dataframe(spa_left_wall)
df_rw = get_dataframe(spa_right_wall)
...
#df = px.data
fig = px.line_3d(data, x='Car Coord X', y='Car Coord Y', z='Car Coord Z', color='Driver', title= 'XYZ Position')
...
fig3 = px.line(data, x='Distance', y='Steering Angle', color = 'Driver', title='Steering Angle Against Distance')
fig3.show()
 
# brake pedal position  ------------------------------------------

fig4 = px.line(data, x='Distance', y='Brake Pos', color='Driver', title='Brake Pos Against Distance')
fig4.show()

# delta t -------------------------------------------

fig5 = px.line(data, x='Distance', y='Time', color='Driver', title='Delta T')
fig5.show()

# speed ------------------------------------------------

fig6 = px.line(data, x='Distance', y='Ground Speed', color='Driver', title='Speed')
fig6.show()

# g-g diagram -----------------------------------------------

fig7 = px.scatter(data, x='CG Accel Lateral', y='CG Accel Longitudinal', color='Driver', title='G-G')
fig7.update_layout( xaxis=dict(range=[-3,3], title='X Axis Accel (ms^-2)') , yaxis=dict(range=[-3,3], title='Y Axis Accel (ms^-2)') )
fig7.show()
"""
...
fig2 = px.line(data_long, x='Distance', y='Tire Pressure', color='Wheel', title='Tire Pressure Over Distance')
fig2.show()
...
# steering angle against distance chart
fig4 = px.line(data, x='Distance', y='Steering Angle', color = 'Driver' title='Steering Angle Against Distance')
fig4.show()
 


fig6 = px.line(data, x='Distance', y='Gear', title='Gear Against Distance')
fig6.show()
"""
